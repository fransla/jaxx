 ...
 *  { cid: CID, service: SERVICE, account: ACCOUNT [, priority: PRIORITY] [, cache: CACHE_INFO ] }
 *
 *  Todo:
 ...
 *    - service/account verification: for example, ensure a btc address is a valid address
 */
 ...
 *  getAccount(service)
 *
 *  Retruns the primary account for a service for a contact or null if
...
Contact.prototype.getAccount = function(service) {
    ...
    for (var i = 0; i < this._records.length; i++) {
        ...
        if (kv.service === service && (result === null || (kv.priority && kv.priority > priority))) {
            result = kv.account;
            if (kv.priority) {
...
/**
 *  getAllAccounts(service)
 *
 *  Finds all accounts for a service for a contact. If no accounts are
...
/**
 *  addAccount(service, account)
 *
 *  Adds an account for a service to a contact. If link is specified,
 ...
 *
 *  Duplicates accounts for a service are ignored.
 */
Contact.prototype.addAccount = function(service, account, link) {
    ...
    if (!service || !account || !Contact.services[service]) {
        console.log('Bad service/account: service=' + service + ", account=" + account);
        return false;
    }
...
/**
 *  refreshAccount(service [,account] [, callback])
 *
 *  Refreshes the content of a service (currently only onename) optionally
...
/**
 *  removeAccount(service [, account])
 *
 *  Remove an account from the contact, or all accounts for a service if
...
Contact.prototype.removeAccount = function(service, account) {
    ...
    for (var i = this._records.length - 1; i >= 0; i--) {
        ...
        if (account) {
            if (kv.service === service) {
                if (kv.account === account && !kv.link) {
                    // removing a specific account from a service; this is that
                    kill = true;
                }
        ...
        } else if (service === kv.service) {
            // removing all of a service; this is that service
            kill = true;
 ...
 *  until at least one service is added to the contact.
 */
 ...
 *    - service (one of: "any" (default), "bitcoin", "name", "onename", "twitter")
...
ContactDatabase.prototype.lookup = function(query) {
    ...
    for (var i = 0; i < this._storage.length; i++) {
        ...
        if ((query.service === 'any' || value.service === query.service) && check(query.value, value.account)) {
            cids[value.cid] = true;
        }
    }
